...
def get_positioning_recommendation(phase: str) -> str:
    ...
    recos = {
        "stagflation": (
            ...
            "  • Oro, argento — inflation hedge + acquisti banche centrali\n"
            "  • Energia (XLE) + commodity producers (miners)\n"
            "  • Difesa EU (Rheinmetall, Leonardo, Safran)\n"
            "  • TIPS / BTP€i — protezione reale dal CPI\n"
            "  • Cash corto (T-bill, XEON) — opzionalità senza duration\n"
            "  • Value vs Growth — il premio di rischio si sposta sui flussi certi\n\n"
            "[bold red]Sottopesare / evitare:[/bold red]\n"
            "  • Bond lunghi (duration distrugge capital a inflazione alta)\n"
            "  • Consumer Discretionary (XLY) — potere d'acquisto eroso\n"
            "  • REIT (XLRE) — sensibili ai tassi reali\n"
            "  • High Yield — spread si allargano in stagflazione\n"
            "  • Growth tech a multipli alti — DCF colpito da tassi elevati\n"
            "  • EM bonds — dollaro forte, tightening globale"
        ),
        "goldilocks": (
            "[bold green]Sovrappesare:[/bold green]\n"
            "  • Equity growth (Tech, Discretionary) — espansione multipli\n"
            "  • Small e mid cap — beneficiano di costo del credito basso\n"
            "  • Credit IG — carry positivo, rischio spread contenuto\n"
            "  • Equity internazionale / EM — dollaro stabile, crescita globale\n"
            "  • REIT — tassi stabili, domanda immobiliare solida\n\n"
            "[bold red]Sottopesare / evitare:[/bold red]\n"
            "  • Cash — drag da opportunity cost\n"
            "  • Difensivi puri (Staples, Utilities) — sottoperformano in risk-on\n"
            "  • Oro — nessuna urgenza come hedge\n"
            "  • Long duration eccessiva — rendimenti stabili o in salita moderata"
        ),
        "reflation": (
            "[bold green]Sovrappesare:[/bold green]\n"
            "  • Ciclici (Industrials, Financials, Materials) — beneficiano della domanda\n"
            "  • Energy + commodity — prezzi input in rialzo\n"
            "  • Small cap value — sensitivity alla crescita nominale\n"
            "  • TIPS / bond inflation-linked — protezione CPI crescente\n"
            "  • Value vs Growth — rotazione da duration a ciclici\n\n"
            "[bold red]Sottopesare / evitare:[/bold red]\n"
            "  • Long duration (bond > 10y) — prezzi scendono con yield in rialzo\n"
            "  • Staples — crescono meno del mercato in reflazione\n"
            "  • Mega cap growth — multipli compressi da tassi reali in rialzo\n"
            "  • Cash — inflazione erode il potere d'acquisto"
        ),
        "late_cycle": (
            "[bold green]Sovrappesare:[/bold green]\n"
            "  • Quality defensives (Staples, Healthcare) — flussi di cassa stabili\n"
            "  • Gold — hedge pre-recessione e reale sotto pressione\n"
            "  • T-bill e cash short (< 1y) — yield attraente, nessun rischio duration\n"
            "  • Minimum volatility / Quality factor — protezione drawdown\n"
            "  • IG credit corto — carry senza prendere spread HY\n\n"
            "[bold red]Sottopesare / evitare:[/bold red]\n"
            "  • High beta e small cap speculativo\n"
            "  • High Yield — spread si allargano prima della recessione\n"
            "  • Ciclici (Industrials, Materials) — earnings decelerano\n"
            "  • Financials — curva invertita comprime i margini bancari\n"
            "  • EM equity — flight to quality penalizza i mercati periferici"
        ),
        "disinflation": (
            "[bold green]Sovrappesare:[/bold green]\n"
            "  • Long duration treasuries (> 10y) — rally da calo dei rendimenti\n"
            "  • Quality defensives (Healthcare, Utilities) — dividend yield attraente\n"
            "  • Gold — tassi reali calanti sono il miglior driver per l'oro\n"
            "  • IG credit lungo — spread si comprimono in risk-off iniziale\n"
            "  • Dividend growers — stabilità in contrazione\n\n"
            "[bold red]Sottopesare / evitare:[/bold red]\n"
            "  • Ciclici — earnings colpiti dalla contrazione della domanda\n"
            "  • Commodity — domanda globale in calo\n"
            "  • Banche — NIM compressi, crediti in sofferenza in aumento\n"
            "  • High Yield — spread si allargano con aumento defaults\n"
            "  • EM equity — risk aversion globale"
        ),
        "transition": (
            "[white]Segnali misti — nessuna regola prevalente.[/white]\n\n"
            "[bold green]Azioni prudenti:[/bold green]\n"
            "  • Mantenere posizioni esistenti senza aumentare il beta\n"
            "  • Aumentare cash / T-bill per opzionalità\n"
            "  • Evitare nuove posizioni direzionali forti\n"
            "  • Monitorare CPI, ISM e curva per segnale direzionale\n\n"
            "[bold red]Evitare:[/bold red]\n"
            "  • Leverage e concentrazione\n"
            "  • Posizioni illiquide senza thesis chiara"
        ),
    }
